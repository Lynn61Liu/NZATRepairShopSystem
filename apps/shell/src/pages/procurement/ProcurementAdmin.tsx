...
export function ProcurementAdmin() {
  ...
  const filteredProducts = products.filter(p => {
    const isTool = /工具|tool/i.test(p.categoryName || '');
            ...
            <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
              <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <div className="flex items-center gap-3">
                  <input type="text" placeholder="🔍 Search by name, code, or specification..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 w-64" />
                  <button onClick={handleSmartScan} className="px-3 py-1.5 text-sm font-bold text-amber-700 bg-amber-100 rounded-lg hover:bg-amber-200 transition shadow-sm border border-amber-200 flex items-center gap-1">🤖 Smart Restock Scan</button>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleOpenAdd('consumable')} className="px-3 py-1.5 text-sm font-bold text-blue-700 bg-blue-100 rounded-lg hover:bg-blue-200 transition shadow-sm border border-blue-200">➕ New Consumable</button>
                  <button onClick={() => handleOpenAdd('tool')} className="px-3 py-1.5 text-sm font-bold text-purple-700 bg-purple-100 rounded-lg hover:bg-purple-200 transition shadow-sm border border-purple-200">🔧 New Tool</button>
                  <input type="file" accept=".csv" ref={fileInputRef} onChange={handleFileUpload} className="hidden" />
                  <button onClick={() => fileInputRef.current?.click()} className="px-3 py-1.5 text-sm font-bold text-green-700 bg-green-100 rounded-lg hover:bg-green-200 transition shadow-sm border border-green-200">📥 Bulk Import</button>
                </div>
              </div>
              <div className="flex-1 overflow-auto p-0">
                {loadingProducts ? <p className="text-gray-500 text-center py-10">Loading data...</p> : (
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200 text-xs text-gray-500 uppercase sticky top-0 z-10">
                        ...
                        <th className="p-3 font-semibold">Category / Supplier</th>
                        <th className="p-3 font-semibold text-right">Purchase Price</th>
                        <th className="p-3 font-semibold text-right">Current Status / Stock</th>
                            ...
                            <td className="p-3 flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-medium text-gray-800 flex items-center gap-1">
                                  {isTool && <span title="Tool">🔧</span>} {p.name}
                                </div>
                    ...
                    <tr key={p.id} className="hover:bg-purple-50/30 transition">
                      ...
                      <td className="p-3 text-center">
                        {!hasInput ? <span className="text-gray-300">-</span> : 
                         consumed === 0 ? <span className="text-gray-400 font-bold">➖ No usage</span> : 
                         consumed > 0 ? <span className="text-blue-600 font-bold">⬇️ Used {consumed}</span> :
                         <span className="text-red-600 font-bold">⚠️ Exception: {Math.abs(consumed)} extra</span>
                        }
                      </td>
                    </tr>
                  );
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="px-6 py-4 border-b border-amber-100 bg-amber-50 flex justify-between items-center">
              <div>
                <h3 className="font-bold text-lg text-amber-900 flex items-center gap-2">🤖 Smart Restock Radar</h3>
                <p className="text-xs text-amber-700 mt-1">The system scanned all items where current stock + in-transit stock is below the alert threshold</p>
              </div>
            ...
            <div className="overflow-auto p-6 flex-1 bg-gray-50">
              {smartRestockItems.length === 0 ? (
                <div className="text-center py-10"><p className="text-gray-500">🎉 Great news! No items are currently below their alert stock level, so no restocking is needed.</p></div>
              ) : (
                  ...
                  {smartRestockItems.map((item: any, idx: number) => (
                    <div key={idx} className="bg-white border border-gray-200 p-4 rounded-lg flex items-center justify-between shadow-sm">
                      <div>
                        ...
                        <div className="text-sm text-gray-500 mt-1">Current stock: <strong className="text-red-500">{item.currentStock}</strong> | In transit: <strong>{item.inTransitStock}</strong> | Alert level: <strong>{item.minStockAlert}</strong></div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-gray-600">Suggested restock qty:</span>
                        <input 
                          type="number" 
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3 bg-white">
              <button onClick={() => setIsSmartModalOpen(false)} className="px-4 py-2 font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 transition">Not now</button>
              {smartRestockItems.length > 0 && (
                <button onClick={handleApplySmartRestock} className="px-6 py-2 font-bold text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition shadow-lg">🚀 Generate procurement requests</button>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0">
              <h3 className="font-bold text-lg text-gray-800">
                {/工具|tool/i.test(formData.categoryName) ? '🔧 Edit / Create Tool' : '📦 Edit / Create Consumable'}
              </h3>
            ...
            <div className="p-6 flex flex-col gap-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-4 border-t pt-4">
                ...
                {!/工具|tool/i.test(formData.categoryName) ? (
