...
export function WorkLogTable({
  ...
  const groupedLogs = useMemo(() => {
    ...
    const logsByDate = new Map<string, WorklogEntry[]>();
    ...
    return Array.from(logsByDate.entries())
      ...
      .map(([workDate, dateLogs]) => {
        const latestLogByStaff = new Map<string, number>();
        const totalsByStaff = new Map<string, number>();
        const logsByStaff = new Map<string, WorklogEntry[]>();

        dateLogs.forEach((log) => {
          const createdAt = new Date(log.created_at).getTime();
          const prevLatest = latestLogByStaff.get(log.staff_name) ?? 0;
          if (createdAt > prevLatest) {
            latestLogByStaff.set(log.staff_name, createdAt);
          }

          totalsByStaff.set(
            log.staff_name,
            (totalsByStaff.get(log.staff_name) ?? 0) + calculateDuration(log.start_time, log.end_time)
          );

          const entries = logsByStaff.get(log.staff_name) ?? [];
          entries.push(log);
          logsByStaff.set(log.staff_name, entries);
        });

        const staffGroups = Array.from(logsByStaff.entries())
          .sort(([staffA], [staffB]) => {
            const latestDiff = (latestLogByStaff.get(staffB) ?? 0) - (latestLogByStaff.get(staffA) ?? 0);
            if (latestDiff !== 0) return latestDiff;

            return (staffOrder.get(staffA) ?? Number.MAX_SAFE_INTEGER) -
              (staffOrder.get(staffB) ?? Number.MAX_SAFE_INTEGER);
          })
          .map(([staffName, staffLogs]) => ({
            staffName,
            totalHours: totalsByStaff.get(staffName) ?? 0,
            logs: [...staffLogs].sort(
        ...
        return {
          workDate,
          summary: staffGroups.map((group) => ({
            staffName: group.staffName,
            totalHours: formatGroupHours(group.totalHours),
          })),
          staffGroups,
        };
      });
  ...
  useEffect(() => {
    setCollapsedDates((prev) => {
      ...
      groupedLogs.forEach((group, index) => {
        next[group.workDate] = prev[group.workDate] ?? index !== 0;
      });
      return next;
    });
  }, [groupedLogs]);
        ...
        <table className="w-full min-w-[1560px] border-collapse">
          ...
          <tbody>
            {pagination.pageRows.map((group) => {
              const isCollapsed = collapsedDates[group.workDate] ?? false;
              const ToggleIcon = isCollapsed ? ChevronRight : ChevronDown;

              return (
                <Fragment key={group.workDate}>
                  <tr key={`${group.workDate}-header`} className="border-b border-slate-200 bg-slate-100">
